{{- define "integrations.memcached.type.metrics" }}true{{- end }}

{{- define "integrations.memcached.module.metrics" }}
declare "memcached_integration" {
  argument "metrics_destinations" {
    comment = "Must be a list of metric destinations where collected metrics should be forwarded to"
  }
  {{- range $instance := $.Values.memcached.instances }}
    {{- include "integrations.memcached.include.metrics" (deepCopy $ | merge (dict "instance" $instance)) | nindent 2 }}
  {{- end }}
}
{{- end }}

{{- define "integrations.memcached.include.metrics" }}
{{- $defaultValues := "integrations/memcached-values.yaml" | .Files.Get | fromYaml }}
{{- with mergeOverwrite $defaultValues .instance (dict "type" "integration.memcached") }}
...
{{- end }}
prometheus.exporter.memcached {{ include "helper.alloy_name" .name | quote }} {
...
prometheus.scrape {{ include "helper.alloy_name" .name | quote }} {
  targets    = prometheus.exporter.memcached.{{ include "helper.alloy_name" .name }}.targets
