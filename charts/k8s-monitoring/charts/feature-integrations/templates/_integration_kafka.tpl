{{- define "integrations.kafka.validate" }}
  ...
  {{- if eq $type "metrics" }}
    ...
    {{- range $idx, $instance := $integration.instances }}
      ...
      {{- if empty $mergedValues.name }}
        {{- fail (printf "kafka instance %d is missing the 'name' field" $idx) }}
      {{- end }}
    {{- end }}
  {{- end }}
{{- end }}

{{- define "secrets.list.integration.kafka" }}
- exporter.auth.username
- exporter.auth.password
{{- end }}

{{- define "integrations.kafka.type.logs" }}false{{- end }}
