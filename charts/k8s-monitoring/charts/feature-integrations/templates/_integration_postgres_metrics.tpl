{{- define "integrations.postgres.type.metrics" }}true{{- end }}

{{- define "integrations.postgres.module.metrics" }}
declare "postgres_integration" {
  argument "metrics_destinations" {
    comment = "Must be a list of metric destinations where collected metrics should be forwarded to"
  }
  {{- range $instance := $.Values.postgres.instances }}
    {{- include "integrations.postgres.include.metrics" (deepCopy $ | merge (dict "instance" $instance)) | nindent 2 }}
  {{- end }}
}
{{- end }}

{{- define "integrations.postgres.include.metrics" }}
{{- $defaultValues := "integrations/postgres-values.yaml" | .Files.Get | fromYaml }}
{{- with mergeOverwrite $defaultValues .instance (dict "type" "integration.postgres") }}
