{{- define "integrations.apache.type.metrics" }}true{{- end }}

{{- define "integrations.apache.module.metrics" }}
declare "apache_integration" {
  argument "metrics_destinations" {
    comment = "Must be a list of metric destinations where collected metrics should be forwarded to"
  }
  {{- range $instance := $.Values.apache.instances }}
    {{- include "integrations.apache.include.metrics" (deepCopy $ | merge (dict "instance" $instance)) | nindent 2 }}
  {{- end }}
}
{{- end }}

{{- define "integrations.apache.include.metrics" }}
{{- $defaultValues := "integrations/apache-values.yaml" | .Files.Get | fromYaml }}
{{- with mergeOverwrite $defaultValues .instance (dict "type" "integration.apache") }}
...
{{- end }}
prometheus.exporter.apache {{ include "helper.alloy_name" .name | quote }} {
  scrape_uri = {{ .exporter.scrapeURI | quote }}
}
...
prometheus.scrape {{ include "helper.alloy_name" .name | quote }} {
  targets    = prometheus.exporter.apache.{{ include "helper.alloy_name" .name }}.targets
